...
/**
 * Default implementation of {@link CiqValidator}.
 */
public class CiqValidatorImpl implements CiqValidator {

    private static final Logger log = LoggerFactory.getLogger(CiqValidatorImpl.class);

    private final ObjectMapper mapper;

    public CiqValidatorImpl() {
        this.mapper = new ObjectMapper();
        this.mapper.enable(SerializationFeature.INDENT_OUTPUT);
    }

    @Override
    public ValidationReport validate(String ciqJsonDir, String nodeType, String activity,
                                     String rulesFilePath, String outputDir,
                                     List<ReportFormat> formats,
                                     String mopJsonOutputDir) throws IOException {
        log.info("=== CIQ Validator ===");
        log.info("JSON dir:  {}", ciqJsonDir);
        ...
        CiqDataStore store = new JsonFileCiqDataStore(ciqJsonDir, nodeType, activity);
        CiqIndex index = store.getIndex();
        ValidationReport report = new CiqValidationEngine(store, rules).validate(nodeType, activity);
        ...
        // Segregate JSON by child order if validation passed and output dir is provided
        if ("PASSED".equals(report.getStatus()) && mopJsonOutputDir != null) {
            log.info("Segregating JSON by child order into: {}", mopJsonOutputDir);
            segregateByChildOrder(store, index, nodeType, activity, mopJsonOutputDir);
        }
     ...
     * </ul>
     * File names use the child-order postfix convention from {@link FileNamingUtil}.
     */
    private void segregateByChildOrder(CiqDataStore store, CiqIndex index,
        ...
        for (NodeEntry entry : index.getEntries()) {
            ...
            // Create child-order subfolder
            File childDir = new File(mopJsonOutputDir, childOrder);
            if (!childDir.exists()) childDir.mkdirs();

            // Write filtered index JSON (single entry for this child order)
            ...
            // Write per-table JSONs filtered to this node
            for (String tableName : entry.getTables()) {
                ...
                // Filter rows to rows belonging to this node
                ...
                String sheetFileName = FileNamingUtil.sheetFileName(nodeType, activity, tableName, childOrder);
                mapper.writeValue(new File(childDir, sheetFileName), filteredSheet);
                log.info("  [{}] table '{}' ({} rows) → {}", childOrder, tableName,
                        filteredRows.size(), sheetFileName);
            }
        }
    }
}
