...
/**
 * Validates {@code type: date} columns.
 *
 * <p>Checks that the value is a parseable calendar date using the configured
 * {@code format} pattern (default: {@code yyyy-MM-dd}).
 *
 ...
 * START_DATE:
 *   type: date
 *   format: yyyy-MM-dd      # optional; this is the default
 * </pre>
 */
public class DateValidator implements CellValidator {
    ...
    public List<ValidationError> validate(CiqRow row, String colName, String value,
        ...
        if (!rule.isDateType()) return Collections.emptyList();
        ...
        try {
            ...
            return Collections.singletonList(new ValidationError(
                    ...
                    "Value '" + value + "' is not a valid date; expected format: " + fmt));
        }
    }
}
